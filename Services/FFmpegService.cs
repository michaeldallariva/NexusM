...
public class FFmpegService
{
    ...
    public void LocateBinaries()
    {
        ...
        // 4. Common system locations (Windows)
    ...
    public async Task DownloadIfMissingAsync()
    {
        ...
        if (!OperatingSystem.IsWindows())
        {
            _logger.LogWarning("Auto-download is only supported on Windows. Install ffmpeg via your package manager.");
            return;
        }

        // Download from BtbN GitHub releases (stable 'latest' tag, GPL static build)
        ...
        try
        {
            ...
            if (extracted > 0)
            {
                ...
                // Re-locate binaries now that they're in place
                LocateBinaries();
            }
            else
            {
        ...
        {
            _logger.LogError(ex, "Failed to download FFmpeg. You can manually place ffmpeg.exe and ffprobe.exe in: {Path}", targetDir);
        }
        finally
        {
            // Clean up temp zip
