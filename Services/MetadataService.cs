        ...
        try
        {
            ...
            foreach (var video in videos)
            {
                try
                {
                    ...
                    // Force CastJson rebuild by temporarily clearing it if it lacks actorId
                    var origCastJson = video.CastJson;
                    bool hadActorIds = false;
                    if (!string.IsNullOrEmpty(origCastJson))
                    {
                        try
                        {
                            using var cjDoc = JsonDocument.Parse(origCastJson);
                            if (cjDoc.RootElement.ValueKind == JsonValueKind.Array && cjDoc.RootElement.GetArrayLength() > 0)
                                hadActorIds = cjDoc.RootElement[0].TryGetProperty("actorId", out _);
                        }
                        catch { }
                    }
                    ...
                    if (!hadActorIds)
                    {
                        video.CastJson = null; // Clear so ApplyMetadataAsync rebuilds it
