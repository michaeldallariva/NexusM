...
public class VideoScannerService
{
    ...
    private async Task ProcessVideoFileAsync(string filePath)
    {
        try
        {
            // Normalize path for consistent DB lookups
            filePath = Path.GetFullPath(filePath);

            // Prevent two threads from inserting the same file concurrently
            ...
            var existing = await db.Videos.FirstOrDefaultAsync(v => v.FilePath.ToLower() == normalizedPath);
            ...
            if (existing != null)
            {
                ...
                // Only overwrite user-curated fields when the record has NOT been manually edited
            ...
            else
            {
                ...
                catch (DbUpdateException)
                {
                    ...
                    _logger.LogDebug("Skipping duplicate video: {File}", filePath);
                    return;
                }
    ...
    public async Task GenerateAllThumbnailsAsync()
    {
        ...
        _logger.LogInformation("Generating thumbnails for {Count} videos ({Total} total)", toGenerate.Count, videos.Count);

        foreach (var video in toGenerate)
        {
