...
public class MusicVideoScannerService
{
    ...
    private async Task ProcessVideoFileAsync(string filePath)
    {
        try
        {
            ...
            var existing = await db.MusicVideos.FirstOrDefaultAsync(v => v.FilePath == filePath);
    ...
    public async Task GenerateAllThumbnailsAsync()
    {
        ...
        _logger.LogInformation("Generating thumbnails for {Count} music videos", videos.Count);
