...
const App = {
    ...
    navigate(page) {
        ...
        const titles = {
            ...
            recent: 'Recently Added', mostplayed: 'Most Played',
    ...
    async renderAnalysis(el) {
        ...
        if (storageItems.length > 0) {
            ...
            // Stacked bar
            html += `<div class="an-stacked-bar">`;
            storageItems.forEach(s => {
                const pct = t.totalSize > 0 ? (s.size / t.totalSize * 100) : 0;
                if (pct > 0) html += `<div class="an-stacked-seg" style="width:${pct}%;background:${s.color}" title="${s.name}: ${this.formatSize(s.size)} (${pct.toFixed(1)}%)"></div>`;
            });
            html += `</div><div class="an-stacked-legend">`;
            storageItems.forEach(s => {
                const pct = t.totalSize > 0 ? (s.size / t.totalSize * 100).toFixed(1) : 0;
                html += `<span class="an-legend-item"><span class="an-legend-dot" style="background:${s.color}"></span>${s.name}: ${this.formatSize(s.size)} (${pct}%)</span>`;
            });
            html += `</div></div>`;
        }
        ...
        if (t.totalVideos > 0 || t.totalMv > 0) {
            ...
            if (t.totalVideos > 0) {
                html += `<div class="an-panel"><h3 class="an-panel-title" style="color:${C.movies}">${this.t('analysis.moviesTvQuality')}</h3>
                    ...
                    ${barChart(data.videos.resolutions, t.totalVideos, C.movies)}
                </div>`;
            }
            ...
            if (t.totalMv > 0) {
                html += `<div class="an-panel"><h3 class="an-panel-title" style="color:${C.mv}">${this.t('analysis.musicVideosQuality')}</h3>
                    ...
                    ${barChart(data.musicVideos.resolutions, t.totalMv, C.mv)}
                </div>`;
            }
        ...
        if (hasPics || hasEbooks) {
            if (hasPics) {
                ...
                html += '<div class="an-panels-row">';
                html += `<div class="an-panel"><h3 class="an-panel-title">Formats & Resolutions</h3>
                    ${barChart(data.pictures.formats, t.totalPictures, C.pictures)}
                    <div style="margin-top:12px;padding-top:12px;border-top:1px solid rgba(255,255,255,.06)">
                    ${barChart(data.pictures.resolutions, t.totalPictures, C.pictures)}
                    </div>
                </div>`;
            ...
            if (hasEbooks) {
                ...
                html += '<div class="an-panels-row">';
                html += `<div class="an-panel"><h3 class="an-panel-title">Formats</h3>${barChart(data.ebooks.formats, t.totalEbooks, C.ebooks)}</div>`;
    ...
    async renderSettings(el) {
        ...
        <div class="setting-row"><span class="setting-label">Run on Windows Startup</span><span class="setting-value">${toggle('runOnStartup', config.runOnStartup)} <span class="setting-hint">Automatically start NexusM when Windows starts</span></span></div>
        </div>`;
        ...
        html += `<div class="settings-section">
            ...
            <div class="setting-row"><span class="setting-label">${this.t('settings.theme')}</span><span class="setting-value">${select('theme', config.theme, [{v:'dark',l:this.t('settings.dark')},{v:'blue',l:this.t('settings.blue')},{v:'purple',l:this.t('settings.purple')}])}</span></div>
        ...
        html += `<div class="settings-section">
            ...
            <div class="setting-row"><span class="setting-label">${this.t('settings.ffmpegStatus')}</span><span class="setting-value">${config.ffmpegAvailable
                ...
                : `<span style="color:var(--warning)">${this.t('status.notInstalled')}</span>
                   ${isAdmin ? `<button class="ffmpeg-download-btn" id="btn-download-ffmpeg" onclick="App.downloadFfmpeg()">${this.t('btn.downloadFfmpeg')}</button>` : ''}`}</span></div>
        </div>`;
