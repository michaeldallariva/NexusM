...
<tbody>{tableRows}</tbody></table>
    ...
    public IActionResult GetConfigInfo()
    {
        ...
        {
            // Server
            version = "BETA 0.96 (New World Order Edition)",
    ...
    public IActionResult ShutdownHost()
    {
        ...
        Task.Run(async () =>
        {
            ...
            Process.Start(new ProcessStartInfo("shutdown", "/s /t 5 /c \"NexusM: Shutdown requested from Web UI\"")
            {
                CreateNoWindow = true,
    ...
    public IActionResult RebootHost()
    {
        ...
        {
            await Task.Delay(2000); // Give response time to reach client
            Process.Start(new ProcessStartInfo("shutdown", "/r /t 5 /c \"NexusM: Reboot requested from Web UI\"")
            {
                CreateNoWindow = true,
                UseShellExecute = false
            });
